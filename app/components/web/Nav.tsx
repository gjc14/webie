...
const CustomNav = ({ children, to }: { children: React.ReactNode; to: string }) => {
	return (
		<NavLink to={to} rel="nofollow" aria-label={`link to ${to}`} className="block overflow-hidden">
			{({ isActive }) => {
				return (
					<motion.div
						whileHover={{ y: -20 }}
					...
					>
						<span className={`flex h-[20px] items-center ${isActive ? 'text-primary' : ''}`}>
							{children}
						</span>
						<span className={`flex h-[20px] items-center text-primary ${isActive ? 'text-primary' : ''}`}>
							{children}
						</span>
					</motion.div>
				)
